        ...
        try:
            ...
            # If not...
                ...
                try:
                    ...
                    print("\nThe input provided is not available. If it is a number you provided, select between 1, 2 or 3")
                    print()
            
        except ValueError:
            print("\n:) NOOOOOOOOO!!!. Try again.  Enter a valid city name or number to continue...")
        except KeyboardInterrupt:
            return
    print(f"\nGREAT, YOU SELECTED {city}")
        # get user input for month (all, january, february, ... , june)
    while True:
        try:
            month = (
                input('\nEnter a month (all, january, february, ... , june):>> ').strip()).lower()
            ...
            # If not...
                ...
                try:
                    ...
                    print("\nThe input provided is not available. If it is a number you provided, select between 0, 1, 2, 3, 4, 5 or 6 for (all, january, february, ... , june)\n")
                    
        except ValueError:
            print("\n:) NOOOOOOOOO!!!  Try again. Enter a valid month name or number to continue...")
        except KeyboardInterrupt:
            return 
    print(f"\nYOU ARE ALMOST THERE. YOU SELECTED {month}")
        ...
        try:
            ...
            # If not...
                ...
                try:
                    ...
                    int_day = int(input('\nOops!!. There may be a typo in there. \
                        You use can enter 0, 1, 2, 3, 4, 5, 6 or 7 for (all, monday, tuesday, ... sunday) respectively:>> '))
                    # Check whether what the user entered is a number, may raise KeyError 
                    day = switch_to_number(int_day, 'day')
                    break
                except KeyError:
                    print("\nThe input provided is not available. If it is a number you provided, select between 0, 1, 2, 3, 4, 5, 6 or 7 for (all, monday, tuesday, ... sunday)\n")
                    
        except ValueError:
            print("\n:) NOOOOOOOOO!!!  Try again. Enter a valid day name or number to continue...")
        except KeyboardInterrupt:
            return
    print(f"\nEXECELLENT!. YOU ARE ON TRACK. YOU SELECTED {day}")
    
    print('-'*40)
    print(f"\nYOUR SEARCH QUERIES ARE CITY: {city}, MONTH: {month} and DAY: {day} \n")
...
def station_stats(read_city):
    ...
    print(f"\nThe most commonly used start station from the given selections is:  {common_use_start_station}\n")
    # display most commonly used end station
    common_use_end_station = read_city['End Station'].mode()[0]
    print(f"\nThe most commonly used end station from the given fitered data is: {common_use_end_station}\n")
    # display most frequent combination of start station and end station trip
    start_end_station = (read_city['Start Station'] + '||' + read_city['End Station']).mode()[0]
    print(f"\nThe most frequent combination of start station and end station trip is :  {start_end_station.split('||')}\n")
...
def trip_duration_stats(read_city):
    ...
    print(f"\nThe mean travel time selections is: {mean_travel_time}\n")
...
def user_stats(read_city, city):
    ...
    print(f"\nThe total counts of user types from selections is:\n{count_user_types}")
    # Display counts of gender
    if 'Gender' in read_city.columns:
        count_gender = read_city['Gender'].value_counts()
        print(f"\nThe count of user gender from Selection is:\n{count_gender}")
    else:
        print(f"\nThe count of user gender from {city} city is not possible because it has no such column as 'Gender'. Skipping now...\n")
    ...
    if 'Birth Year' in read_city.columns:
        ...
        print(f"\nEarliest birth from Selection is: {earliest_birth}\n")
        # Most recent year of birth
        most_recent_birth = read_city['Birth Year'].max()
        print(f"\nMost recent birth from Selection is: {most_recent_birth}\n")
        # Most common year of birth
        most_common_birth = read_city['Birth Year'].mode()[0]
        print(f"\nMost common birth from Selection is: {most_common_birth}\n")
    else:
        print(f"\nThe count stats for Birth Year from {city} city is not possible because it has no such column as 'Birth Year'. Skipping now...\n")
    print("\nThis took %s seconds.\n" % (time.time() - start_time))
    print('-'*40)
    print("D O N E")
...
def user_view_raw_data(read_city):
    ...
    while True:
        ...
        if view_raw_data.lower() == 'yes':
            ...
            initial_view = 0
            while True:
                view_raw_data = input('\nWould you like to view the next five raw data set from this city? Enter yes or no. >>')
